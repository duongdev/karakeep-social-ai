...
export class TwitterAdapter extends BaseAdapter {
  ...
  private async getUserId(): Promise<string> {
    ...
    this.userId = response.data[0].id as string;
    return this.userId;
  }
   ...
   */
  private async fetchBookmarks(userId: string, since?: Date): Promise<Post[]> {
    const posts: Post[] = [];
   ...
   * Fetch user retweets
   */
  private async fetchRetweets(userId: string, since?: Date): Promise<Post[]> {
    const posts: Post[] = [];
    let paginationToken: string | undefined;
